...
public interface HeritageRepository extends JpaRepository<HeritageEntity, Long> {
  ...
  @Query(value = "SELECT h FROM HeritageEntity h WHERE ST_Distance(h.location, :point) < :radiusDistance")
  Optional<List<HeritageEntity>> findWithinDistance(Point point, int radiusDistance);
}
